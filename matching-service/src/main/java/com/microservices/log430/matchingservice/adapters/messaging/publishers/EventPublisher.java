package com.microservices.log430.matchingservice.adapters.messaging.publishers;
...
public class EventPublisher {
    ...
    private final TopicExchange matchingExchange;
    ...
    public EventPublisher(RabbitTemplate rabbitTemplate,
                         TopicExchange matchingExchange,
    ...
    private void publishEvent(OutboxEventEntity event) {
        ...
        TopicExchange exchange = getExchangeForEventType(event.getEventType());
    ...
    private String getRoutingKeyForEventType(String eventType) {
        return switch (eventType) {
            case "ORDER_MATCHED" -> orderMatchedRoutingKey;
            case "ORDER_REJECTED" -> orderRejectedRoutingKey;
    ...
    /**
     * Détermine l'exchange basé sur le type d'événement
     */
    private TopicExchange getExchangeForEventType(String eventType) {
        return switch (eventType) {
            case "ORDER_MATCHED", "ORDER_REJECTED" -> matchingExchange;
            case "NOTIFICATION_SEND" -> notificationExchange;
            default -> throw new IllegalArgumentException("Type d'événement non supporté: " + eventType);
        };
    }
}
