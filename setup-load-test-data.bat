...
docker exec brokerx-postgres-wallet psql -U postgres -d brokerxdb_wallet -c "SELECT COUNT(*) as wallets_created FROM wallet WHERE user_id BETWEEN 1 AND 100;"

echo.
echo Verification des positions d'actions:
docker exec brokerx-postgres-wallet psql -U postgres -d brokerxdb_wallet -c "SELECT COUNT(*) as positions_created FROM stock_position sp JOIN wallet w ON sp.wallet_id = w.id WHERE w.user_id BETWEEN 1 AND 100;"

echo.
echo Exemple de portefeuille (utilisateur 1):
docker exec brokerx-postgres-wallet psql -U postgres -d brokerxdb_wallet -c "SELECT w.user_id, w.balance as cash, sp.symbol, sp.quantity, sp.average_price FROM wallet w LEFT JOIN stock_position sp ON w.id = sp.wallet_id WHERE w.user_id = 1 ORDER BY sp.symbol;"
...
echo ========================================
echo.
echo ✅ 100 utilisateurs de test sont maintenant prets
